...
```powershell
# Сгенерировать Prisma Client (в т.ч. после смены схемы)
npx prisma generate

# Применить схему к базе данных
npm run db:push

# Или применить миграции (рекомендуется для продакшена)
npm run db:migrate
```

> Для личного кабинета с рецептами нужна миграция `20260124120000_add_recipe_favorites` (таблица `recipe_favorites`). Она применится при `npm run db:migrate` или `npx prisma migrate deploy`.
...
## 🧑‍🍳 Личный кабинет (рецепты)

Маршруты: `/dashboard` (мои рецепты), `/dashboard/public`, `/dashboard/favorites`, `/dashboard/history`, `/dashboard/settings` (заглушки).

### ENV для личного кабинета

Используются те же переменные, что и для приложения: `DATABASE_URL`, для авторизации — `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `AUTH_SECRET` (см. `.env.example`).
...
1. **Войти** — авторизация через Google (OAuth).
2. **Создать** — в `/dashboard` нажать «+ Новый рецепт», заполнить название, содержание, переключатель «Публичный», отправить.
3. **Редактировать** — на карточке рецепта нажать иконку карандаша, изменить данные, «Сохранить».
4. **Удалить** — нажать иконку корзины, подтвердить.
5. **Переключить public/private** — на карточке иконка Globe (публичный) / Lock (приватный).
6. **Избранное** — нажать звёздочку на карточке; список в `/dashboard/favorites`.
